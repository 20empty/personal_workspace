...
export default function DeleteDeliverableModal({
  ...
  return (
    <motion.div
      className="fixed inset-0 z-40 grid place-items-center bg-black/45 backdrop-blur-sm"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      onClick={onClose}
    >
      <motion.div
        ...
        className="w-[440px] max-w-[92vw] rounded-3xl border border-[color:var(--border)] bg-[color:var(--panel-strong)] p-6 shadow-2xl shadow-black/40 backdrop-blur-xl"
      >
        ...
        <div className="mt-4 rounded-2xl border border-[color:var(--border)] bg-[color:var(--panel)] p-4 text-sm text-[color:var(--muted)]">
          删除后该交付物会进入归档记录：<span className="text-[color:var(--text)]">{title}</span>
        </div>
        ...
        <div className="mt-6 flex items-center justify-end gap-3">
          ...
          <button
            onClick={onConfirm}
            className="inline-flex items-center gap-2 rounded-2xl bg-rose-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-rose-500/20"
          >
